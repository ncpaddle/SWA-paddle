...
def pytorch2paddle():
    ...
    output_fp = "../swa_paddle/model_paddle.pdparams"
...
def forward_pytorch(args):
    ...
    print('Preparing model')
    model = model_cfg.base(*model_cfg.args, num_classes=10, **model_cfg.kwargs)
    model.load_state_dict(torch.load("./model_pytorch.pt"))
    model.cuda()
    model.eval()
...
def bp_align_pytorch(args):
    ...
    for idx in range(3):
        ...
        loss = criterion(output, fake_label.squeeze())
        loss.backward()
        optimizer.step()
        optimizer.zero_grad()
